...
### 機能追加
- 設定画面に「基本勤務時間」設定を追加
  - 基本始業時間・基本終業時間をtime入力で設定可能
  - 設定した始業時間になると自動で始業＋GPS追跡を開始
  - 設定した終業時間になると自動で終業＋GPS追跡を停止
  - 同日の重複自動始業を防止（autoStartedフラグ）
  - 設定中の時間をサマリー表示（「毎日 08:00 に自動始業・17:00 に自動終業」）
- ダッシュボードに始業時間の変更機能を追加
  - 勤務中の経過時間表示横に「変更」ボタン
  - タップでtime入力に切り替わり、始業時間を修正可能
  - 変更後、GPSが未稼働なら自動で追跡開始

### バグ修正
- 自動始業/終業タイマーのuseEffect依存配列を修正（geoRef使用でタイマー重複登録を防止）
- 自動始業/終業の同一分内での重複発火を防止（lastFiredMinuteチェック追加）
- GpsLogServiceの存在チェック漏れを修正

## [1.3.8] - 2026-02-28

### 機能追加
- カレンダーの勤務/休日データをクラウド同期対応
  - 勤務・休日を変更するたびに自動でVercel Blob Storageにアップロード
  - アプリ起動時・タブ復帰時にクラウドから最新の勤務状態を自動取得
  - 複数端末間で勤務/休日マーキングが同期される
  - API（/api/data）に`workstatus`タイプを追加
  - クラウド側のデータをローカルにマージ（クラウド優先）

## [1.3.7] - 2026-02-28

### 機能追加
- データ管理の編集フォームに支払方法セレクターを追加
  - 現金・未収・DIDI決済の3択をトグルボタンで選択可能
  - 既存データの支払方法を編集・変更可能に
