...
window.GpsLogService = (() => {
  ...
  const RT_STANDBY_MIN_MS = 900000;   // 15分以上で待機確定
  const RT_STANDBY_RIDE_CHECK_MS = 120000; // 待機終了後2分間は乗車チェック猶予
  const RT_STANDBY_MAX_CUMULATIVE = 100; // 累積移動距離100m超で待機キャンセル（流し判定）
  let _rtAnchor = null;   // { lat, lng, startTime, lastTime, prevLat, prevLng, cumDist }
  ...
  function _rtDetectStandby(lat, lng, now) {
    ...
    if (_rtAnchor === null) {
      _rtAnchor = { lat, lng, startTime: now, lastTime: now, prevLat: lat, prevLng: lng, cumDist: 0 };
      return;
    }
    ...
    if (dist <= RT_STANDBY_RADIUS) {
      // 50m圏内だが、直前位置からの移動距離を累積する
      const stepDist = _haversine(_rtAnchor.prevLat, _rtAnchor.prevLng, lat, lng);
      _rtAnchor.cumDist += stepDist;
      _rtAnchor.prevLat = lat;
      _rtAnchor.prevLng = lng;
      _rtAnchor.lastTime = now;

      // 累積移動距離が閾値を超えた → 流し走行とみなしアンカーリセット
      if (_rtAnchor.cumDist > RT_STANDBY_MAX_CUMULATIVE) {
        _rtAnchor = { lat, lng, startTime: now, lastTime: now, prevLat: lat, prevLng: lng, cumDist: 0 };
      }
    } else {
      // 動き始めた → 待機期間を確定するか判定
      const duration = _rtAnchor.lastTime - _rtAnchor.startTime;
      if (duration >= RT_STANDBY_MIN_MS && _rtAnchor.cumDist <= RT_STANDBY_MAX_CUMULATIVE) {
        // 待機確定 → 乗車チェック待ちに入れる
  ...
  function getRealtimeStandbyStatus() {
    ...
    if (duration < RT_STANDBY_MIN_MS) return null; // 15分未満は待機とみなさない
    const catInfo = _classifyStandbyCategory(_rtAnchor.lat, _rtAnchor.lng);
    return {
      lat: _rtAnchor.lat,
      lng: _rtAnchor.lng,
  ...
  /** 既知の場所（駅・病院・ホテル・待機スポット・KNOWN_PLACES）のみ記録。不明な場所はスキップ */
  function _autoRecordVacantStandby(standby) {
    ...
    // 既知の場所以外は自動記録しない（ゴミデータ防止）
    if (standby.category === 'other' && !standby.manualLocationName) {
      if (window.AppLogger) AppLogger.debug(`待機スキップ: 不明な場所 ${standby.durationMin}分 (${standby.lat.toFixed(4)}, ${standby.lng.toFixed(4)})`);
      return;
    }
  ...
  function _classifyStandbyCategory(lat, lng) {
    ...
    // KNOWN_PLACESチェック（旭川市役所、イオンモール等を認識）
    if (window.TaxiApp && TaxiApp.utils && TaxiApp.utils.matchKnownPlace) {
      const knownName = TaxiApp.utils.matchKnownPlace(lat, lng);
      if (knownName) return { category: 'known', categoryLabel: knownName, nearbyName: knownName };
    }
  ...
  function _finalizeStandby(points, startIdx, endIdx) {
    ...
    if (durationMin < 15) return null; // 15分未満は破棄
