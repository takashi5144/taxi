...
window.CalendarPage = () => {
  ...
  return createElement('div', null,
    ...
    selectedDayData && createElement('div', {
          ...
          createElement('div', {
            ...
            createElement('div', {
              ...
              createElement('div', {
                ...
                createElement('span', { className: 'material-icons-round', style: { fontSize: 14 } }, 'smartphone'),
                'DIDI+Uber合計',
                createElement('span', { style: { fontSize: 11, fontWeight: 400, color: 'var(--text-muted)', marginLeft: 4 } }, `${dayDidiEntries.length + dayUberEntries.length}件`)
              ),
              createElement('div', { style: { fontSize: 'var(--font-size-lg)', fontWeight: 700, color: '#ffb74d' } },
                `¥${(dayDidi + dayUber).toLocaleString()}`
              )
            ),
            createElement('div', {
              style: { display: 'flex', gap: '10px', fontSize: 11, color: 'var(--text-muted)', flexWrap: 'wrap' },
            },
              dayDidi > 0 && createElement('span', null, `DIDI: ${dayDidiEntries.length}件 ¥${dayDidi.toLocaleString()}`),
              dayUber > 0 && createElement('span', null, `Uber: ${dayUberEntries.length}件 ¥${dayUber.toLocaleString()}`)
            )
          ),
      ...
      // 追加のシフト記録（1日2件目以降。先頭は上の始業・終業フォームで編集）
      selectedDayData.shifts.length > 1 && createElement('div', {
        ...
        ...selectedDayData.shifts.slice(1).map((s, i) => {
